...
class Promotion{
	...
	{
		$db=Db::getInstance();
		$req= $db->query("INSERT INTO promotion (id_prod, description, date_debut, date_fin, pourcentage) VALUES (".$this->id_prod.",'".$this->description."','".$this->date_debut."','".$this->date_fin."','".$this->pourcentage."') ");
	}
     
    public static function all(){
    	$list =[];
    	$db = Db::getInstance();
    	$req= $db->query('SELECT p.id_prom , p.id_prod,p.description,p.date_debut,p.date_fin,p.pourcentage,pro.nom FROM promotion p INNER JOIN produit pro ON pro.id_produit = p.id_prod ');
    	foreach($req->fetchall() as $temp){
    		...
    		$promotion->setPourcentage($temp['pourcentage']);
    		$promotion->setNomProduit($temp['nom']);
    		$list[]=$promotion;
    	}
    	return $list;
    ...
    public static function findByDescription($description){
    	...
    	$promotion->setDate_debut($temp['date_debut']);
    	$promotion->setDate_fin($temp['date_fin']);
    	$promotion->setCoupon($temp['pourcentage']);
    	$list[]=$promotion;
    }
    ...
    {
    	$db= Db::getInstance();
    	$req= $db->query("UPDATE promotion SET id_prod=".$this->id_prod.",description='".$this->description."',date_debut='".$this->date_debut."',date_fin='".$this->date_fin."',pourcentage='".$this->pourcentage."' where id_prom=".$this->id_prom);
